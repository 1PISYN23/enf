...
class OrderItemInline(admin.TabularInline):
    model = OrderItem
    extra = 0
    field = ("image_preview", "product", "size", "quantity", "price", "get_total_price")
    ...
    def image_preview(self, obj):
        if obj.product.main_image:
            return mark_safe(f"<img src='{obj.main_image.url}' style='max-height: 150px; max-width: 150px; object-fit: cover;' />")
...
class OrderAdmin(admin.ModelAdmin):
    ...
    def get_readonly_fields(self, request, obj=None):  # Объяснить.
        if obj:
